...
{
    return &((struct PyThreadStateImpl *)tstate)->brc;
}

static struct brc_state *
find_brc_state(Bucket *bucket, uintptr_t thread_id)
{
    struct llist_node *node;
    llist_for_each(node, &bucket->threads) {
        struct brc_state *brc = llist_data(node, struct brc_state, node);
...
_Py_queue_object(PyObject *ob, uintptr_t tid)
{
    ...
    struct brc_state *brc = find_brc_state(bucket, tid);
    if (!brc) {
    ...
    // struct PyThreadStateImpl *tstate_impl;
    // tstate_impl = llist_data(brc, struct PyThreadStateImpl, brc);
